...
_leaflet.default.DomUtil = _leaflet.default.extend(_leaflet.default.DomUtil, {
  getMatrixString: function (m) {
    var is3d = _leaflet.default.Browser.webkit3d || _leaflet.default.Browser.gecko3d || _leaflet.default.Browser.ie3d,
...
_leaflet.default.Map.include({
  _newLayerPointToLatLng: function (point, newZoom, newCenter) {
    var topLeft = _leaflet.default.Map.prototype._getNewTopLeftPoint.call(this, newCenter, newZoom).add(_leaflet.default.Map.prototype._getMapPanePos.call(this));

    return this.unproject(point.add(topLeft), newZoom);
  }
});

_leaflet.default.MatrixUtil = {
  ...
  basisToPoints: function (x1, y1, x2, y2, x3, y3, x4, y4) {
    var m = [x1, x2, x3, y1, y2, y3, 1, 1, 1],
        v = _leaflet.default.MatrixUtil.multmv(_leaflet.default.MatrixUtil.adj(m), [x4, y4, 1]);

    return _leaflet.default.MatrixUtil.multmm(m, [v[0], 0, 0, 0, v[1], 0, 0, 0, v[2]]);
  },
  project: function (m, x, y) {
    var v = _leaflet.default.MatrixUtil.multmv(m, [x, y, 1]);

    return [v[0] / v[2], v[1] / v[2]];
  },
  general2DProjection: function (x1s, y1s, x1d, y1d, x2s, y2s, x2d, y2d, x3s, y3s, x3d, y3d, x4s, y4s, x4d, y4d) {
    var s = _leaflet.default.MatrixUtil.basisToPoints(x1s, y1s, x2s, y2s, x3s, y3s, x4s, y4s),
        d = _leaflet.default.MatrixUtil.basisToPoints(x1d, y1d, x2d, y2d, x3d, y3d, x4d, y4d),
        m = _leaflet.default.MatrixUtil.multmm(d, _leaflet.default.MatrixUtil.adj(s));
    /* 
    ...
    return _leaflet.default.MatrixUtil.multsm(1 / m[8], m);
  }
};
_leaflet.default.EditHandle = _leaflet.default.Marker.extend({
  initialize: function (overlay, corner, options) {
    ...
    _leaflet.default.Marker.prototype.initialize.call(this, latlng, markerOptions);
  },
  onAdd: function (map) {
    _leaflet.default.Marker.prototype.onAdd.call(this, map);
...
_leaflet.default.LockHandle = _leaflet.default.EditHandle.extend({
  options: {
    TYPE: 'lock',
...
_leaflet.default.DistortHandle = _leaflet.default.EditHandle.extend({
  options: {
    TYPE: 'distort',
    icon: new _leaflet.default.Icon({
...
_leaflet.default.RotateHandle = _leaflet.default.EditHandle.extend({
  options: {
    TYPE: 'rotate',
    icon: new _leaflet.default.Icon({
  ...
  _onHandleDrag: function () {
    var overlay = this._handled,
        ...
        angle = this._calculateAngle(formerLatLng, newLatLng); //scale = this._calculateScalingFactor(formerLatLng, newLatLng);


    overlay.editing._rotateBy(angle); //overlay.editing._scaleBy(scale);
...
_leaflet.default.ScaleHandle = _leaflet.default.EditHandle.extend({
  options: {
    TYPE: 'scale',
    icon: new _leaflet.default.Icon({
  ...
  _onHandleDrag: function () {
    var overlay = this._handled,
        ...
        scale = this._calculateScalingFactor(formerLatLng, newLatLng); //overlay.editing._rotateBy(angle);
...
_leaflet.default.DistortableImageOverlay = _leaflet.default.ImageOverlay.extend({
  include: _leaflet.default.Mixin.Events,
  options: {
    alt: '',
    height: 200,
    crossOrigin: true
  ...
  onAdd: function (map) {
    ...
    if (this.options.corners) {
      ...
      if (map.options.zoomAnimation && _leaflet.default.Browser.any3d) {
    ...
    _leaflet.default.DomEvent.on(this._image, 'load', function () {
  ...
  onRemove: function (map) {
    ...
    _leaflet.default.ImageOverlay.prototype.onRemove.call(this, map);
  },
  _initImage: function () {
    _leaflet.default.ImageOverlay.prototype._initImage.call(this);

    _leaflet.default.extend(this._image, {
      alt: this.options.alt
    });
  },
  _initImageDimensions: function () {
    var map = this._map,
        originalImageWidth = _leaflet.default.DomUtil.getStyle(this._image, 'width'),
        originalImageHeight = _leaflet.default.DomUtil.getStyle(this._image, 'height'),
        ...
        offset = new _leaflet.default.Point(imageWidth, imageHeight).divideBy(2);

    if (this.options.corners) {
      this._corners = this.options.corners;
    } else {
      this._corners = [map.containerPointToLatLng(center.subtract(offset)), map.containerPointToLatLng(center.add(new _leaflet.default.Point(offset.x, -offset.y))), map.containerPointToLatLng(center.add(new _leaflet.default.Point(-offset.x, offset.y))), map.containerPointToLatLng(center.add(offset))];
    }
  },
  _initEvents: function () {
    ...
    for (var i = 0, l = this._events.length; i < l; i++) {
      _leaflet.default.DomEvent.on(this._image, this._events[i], this._fireMouseEvent, this);
    }
  },
  ...
  _getTranslateString: function (point) {
    ...
    var is3d = _leaflet.default.Browser.webkit3d,
  ...
  _animateZoom: function (event) {
    ...
    if (!_leaflet.default.Browser.gecko) {
      image.style[_leaflet.default.DomUtil.TRANSFORM] = [translation, warp].join(' ');
    }
  },
  getCorners: function () {
  ...
  _calculateProjectiveTransform: function (latLngToCartesian) {
    ...
    return _leaflet.default.MatrixUtil.general2DProjection(0, 0, c[0].x, c[0].y, w, 0, c[1].x, c[1].y, 0, h, c[2].x, c[2].y, w, h, c[3].x, c[3].y);
  }
});
_leaflet.default.DistortableImage = _leaflet.default.DistortableImage || {};
_leaflet.default.DistortableImage.Edit = _leaflet.default.Handler.extend({
  options: {
    opacity: 0.7,
    outline: '1px solid red',
    keymap: {
      68: '_toggleRotateDistort',
      // d
      73: '_toggleIsolate',
      // i
      76: '_toggleLock',
  ...
  addHooks: function () {
    var overlay = this._overlay,
        map = overlay._map,
        i;
    this._lockHandles = new _leaflet.default.LayerGroup();

    for (i = 0; i < 4; i++) {
      this._lockHandles.addLayer(new _leaflet.default.LockHandle(overlay, i, {
        draggable: false
      }));
    }

    this._distortHandles = new _leaflet.default.LayerGroup();

    for (i = 0; i < 4; i++) {
      this._distortHandles.addLayer(new _leaflet.default.DistortHandle(overlay, i));
    }

    this._rotateHandles = new _leaflet.default.LayerGroup();

    for (i = 0; i < 4; i++) {
      this._rotateHandles.addLayer(new _leaflet.default.RotateHandle(overlay, i));
    }

    this._scaleHandles = new _leaflet.default.LayerGroup();
    var topLeftScaleHandle = new _leaflet.default.ScaleHandle(overlay, 0); //topLeftScaleHandle.options.icon.style[L.DomUtil.TRANSFORM] += ' rotateZ(90deg)';

    this._scaleHandles.addLayer(topLeftScaleHandle); //this._scaleHandles.addLayer( new L.ScaleHandle(overlay, 0));


    this._scaleHandles.addLayer(new _leaflet.default.ScaleHandle(overlay, 1));

    this._scaleHandles.addLayer(new _leaflet.default.ScaleHandle(overlay, 2));

    this._scaleHandles.addLayer(new _leaflet.default.ScaleHandle(overlay, 3));
    ...
    _leaflet.default.DomEvent.on(overlay._image, 'click', this._showToolbar, this);
    /* Enable hotkeys. */


    _leaflet.default.DomEvent.on(window, 'keydown', this._onKeyDown, this);
  ...
  removeHooks: function () {
    var overlay = this._overlay,
        map = overlay._map; // L.DomEvent.off(window, 'keydown', this._onKeyDown, this);

    _leaflet.default.DomEvent.off(overlay._image, 'click', this._showToolbar, this); // First, check if dragging exists;
    ...
    _leaflet.default.DomEvent.off(window, 'keydown', this._onKeyDown, this);
  ...
  _rotateBy: function (angle) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      q = new _leaflet.default.Point(Math.cos(angle) * p.x - Math.sin(angle) * p.y, Math.sin(angle) * p.x + Math.cos(angle) * p.y);
  ...
  _enableDragging: function () {
    var overlay = this._overlay,
        map = overlay._map;
    this.dragging = new _leaflet.default.Draggable(overlay._image);
  ...
  _toggleTransparency: function () {
    ...
    _leaflet.default.DomUtil.setOpacity(image, opacity);
  ...
  _showToolbar: function (event) {
    ...
    var raised_point = map.containerPointToLatLng(new _leaflet.default.Point(point.x, point.y - 20));
    raised_point.lng = overlay.getCenter().lng;
    this.toolbar = new _leaflet.default.DistortableImage.EditToolbar(raised_point).addTo(map, overlay);
    overlay.fire('toolbar:created');

    _leaflet.default.DomEvent.stopPropagation(event);
  },
...
_leaflet.default.DistortableImageOverlay.addInitHook(function () {
  this.editing = new _leaflet.default.DistortableImage.Edit(this);

  if (this.options.editable) {
    _leaflet.default.DomEvent.on(this._image, 'load', this.editing.enable, this.editing);
  }
