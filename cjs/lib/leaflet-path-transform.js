...
_leaflet.default.Handler.PathDrag = _leaflet.default.Handler.extend(
/** @lends  L.Path.Drag.prototype */
{
  ...
  addHooks: function () {
    ...
    this._path.options.className = this._path.options.className ? this._path.options.className + ' ' + _leaflet.default.Handler.PathDrag.DRAGGING_CLS : _leaflet.default.Handler.PathDrag.DRAGGING_CLS;

    if (this._path._path) {
      _leaflet.default.DomUtil.addClass(this._path._path, _leaflet.default.Handler.PathDrag.DRAGGING_CLS);
    }
  },
  ...
  removeHooks: function () {
    ...
    this._path.options.className = this._path.options.className.replace(new RegExp('\\s+' + _leaflet.default.Handler.PathDrag.DRAGGING_CLS), '');

    if (this._path._path) {
      _leaflet.default.DomUtil.removeClass(this._path._path, _leaflet.default.Handler.PathDrag.DRAGGING_CLS);
    }
  },
  ...
  _onDragStart: function (evt) {
    ...
    _leaflet.default.DomEvent.stop(evt.originalEvent);

    _leaflet.default.DomUtil.addClass(this._path._renderer._container, 'leaflet-interactive');

    _leaflet.default.DomEvent.on(document, MOVE[eventType], this._onDrag, this).on(document, END[eventType], this._onDragEnd, this);
  ...
  _onDrag: function (evt) {
    _leaflet.default.DomEvent.stop(evt);
  ...
  _onDragEnd: function (evt) {
    ...
    _leaflet.default.DomEvent.off(document, 'mousemove touchmove', this._onDrag, this);

    _leaflet.default.DomEvent.off(document, 'mouseup touchend', this._onDragEnd, this);
    ...
    if (moved) {
      ...
      this._path._containsPoint = _leaflet.default.Util.falseFn;

      _leaflet.default.Util.requestAnimFrame(function () {
        _leaflet.default.DomEvent.skipped({
          type: 'click'
        });
    ...
    if (this._mapDraggingWasEnabled) {
      if (moved) _leaflet.default.DomEvent.fakeStop({
        type: 'click'
      });
  ...
  _transformPoints: function (matrix, dest) {
    ...
    var px = _leaflet.default.point(matrix[4], matrix[5]);

    var crs = path._map.options.crs;
    var transformation = crs.transformation;
    var scale = crs.scale(path._map.getZoom());
    var projection = crs.projection;
    var diff = transformation.untransform(px, scale).subtract(transformation.untransform(_leaflet.default.point(0, 0), scale));
    var applyTransform = !dest;
    path._bounds = new _leaflet.default.LatLngBounds(); // console.time('transform');
    ...
    if (path._point) {
      ...
      if (!_leaflet.default.Util.isArray(latlngs[0])) {
        // polyline
        latlngs = [latlngs];
        dest = [dest];
      }
  ...
  _replaceCoordGetters: function () {
    if (this._path.getLatLng) {
      ...
      this._path.getLatLng = _leaflet.default.Util.bind(function () {
      ...
      this._path.getLatLngs = _leaflet.default.Util.bind(function () {
        return this.dragging._transformPoints(this.dragging._matrix, []);
      }, this._path);
    }
  },
...
_leaflet.default.Handler.PathDrag.makeDraggable = function (layer) {
  layer.dragging = new _leaflet.default.Handler.PathDrag(layer);
  return layer;
};
/**
...
_leaflet.default.Path.prototype.makeDraggable = function () {
  return _leaflet.default.Handler.PathDrag.makeDraggable(this);
};

_leaflet.default.Path.addInitHook(function () {
  if (this.options.draggable) {
    ...
    if (this.dragging) {
      ...
      _leaflet.default.Handler.PathDrag.makeDraggable(this);
...
_leaflet.default.SVG.include(!_leaflet.default.Browser.vml ? {} : {
  /**
   * Reset transform matrix
   */
  _resetTransformPath: function (layer) {
  ...
  transformPath: function (layer, matrix) {
    ...
    if (!skew) {
      skew = _leaflet.default.SVG.create('skew');
...
_leaflet.default.Canvas.include({
  /**
   * Do nothing
   * @param  {L.Path} layer
   */
  ...
  transformPath: function (layer, matrix) {
    ...
    var ctx = this._ctx,
        copyCtx;
    var m = _leaflet.default.Browser.retina ? 2 : 1;
...
_leaflet.default.PathTransform = {};
/**
...
_leaflet.default.PathTransform.pointOnLine = function (start, final, distPx) {
  var ratio = 1 + distPx / start.distanceTo(final);
  return new _leaflet.default.Point(start.x + (final.x - start.x) * ratio, start.y + (final.y - start.y) * ratio);
};
/**
 * Deep merge objects.
 */
...
_leaflet.default.PathTransform.merge = function () {
  var i = 1;
  var key, val;
  var obj = arguments[i];
  ...
  while (obj) {
    ...
    for (key in obj) {
      ...
      if (isObject(val) && isObject(target[key])) {
        target[key] = _leaflet.default.Util.merge(target[key], val);
      } else {
        target[key] = val;
      }
    }
...
_leaflet.default.Matrix = function (a, b, c, d, e, f) {
  /**
   * @type {Array.<Number>}
   */
...
_leaflet.default.Matrix.prototype = {
  /**
   * @param  {L.Point} point
   * @return {L.Point}
   */
  ...
  untransform: function (point) {
    var matrix = this._matrix;
    return new _leaflet.default.Point((point.x / matrix[0] - matrix[4]) / matrix[0], (point.y / matrix[2] - matrix[5]) / matrix[2]);
  },
  ...
  clone: function () {
    var matrix = this._matrix;
    return new _leaflet.default.Matrix(matrix[0], matrix[1], matrix[2], matrix[3], matrix[4], matrix[5]);
  },
  ...
  translate: function (translate) {
    if (translate === undefined) {
      return new _leaflet.default.Point(this._matrix[4], this._matrix[5]);
    }
  ...
  scale: function (scale, origin) {
    if (scale === undefined) {
      return new _leaflet.default.Point(this._matrix[0], this._matrix[3]);
    }

    var scaleX, scaleY;
    origin = origin || _leaflet.default.point(0, 0);
  ...
  rotate: function (angle, origin) {
    ...
    origin = origin || new _leaflet.default.Point(0, 0);
  ...
  _add: function (a, b, c, d, e, f) {
    ...
    if (a && a instanceof _leaflet.default.Matrix) {
      src = a._matrix;
...
_leaflet.default.matrix = function (a, b, c, d, e, f) {
  return new _leaflet.default.Matrix(a, b, c, d, e, f);
};
/**
 * Marker handler
...
_leaflet.default.PathTransform.Handle = _leaflet.default.CircleMarker.extend({
  options: {
  ...
  onAdd: function (map) {
    ...
    if (this._path && this.options.setCursor) {
      // SVG/VML
      this._path.style.cursor = _leaflet.default.PathTransform.Handle.CursorsByType[this.options.index];
    }
  }
});
/**
...
_leaflet.default.PathTransform.Handle.CursorsByType = ['nesw-resize', 'nwse-resize', 'nesw-resize', 'nwse-resize'];
/**
 * @extends {L.Handler.PathTransform.Handle}
 */

_leaflet.default.PathTransform.RotateHandle = _leaflet.default.PathTransform.Handle.extend({
  options: {
...
_leaflet.default.Handler.PathTransform = _leaflet.default.Handler.extend({
  options: {
    rotation: true,
    scaling: true,
    uniformScaling: true,
    ...
    edgesCount: 4,
    handleClass: _leaflet.default.PathTransform.Handle,
    rotateHandleClass: _leaflet.default.PathTransform.RotateHandle
  },
  ...
  initialize: function (path) {
    ...
    this._angle = 0;
    this._scale = _leaflet.default.point(1, 1);
    this._initialDist = 0;
    ...
    this._matrix = new _leaflet.default.Matrix(1, 0, 0, 1, 0, 0);
    this._projectedMatrix = new _leaflet.default.Matrix(1, 0, 0, 1, 0, 0); // ui elements
  ...
  enable: function (options) {
    if (this._path._map) {
      ...
      _leaflet.default.Handler.prototype.enable.call(this);
    }
  },
  ...
  setOptions: function (options) {
    ...
    this.options = _leaflet.default.PathTransform.merge({}, _leaflet.default.Handler.PathTransform.prototype.options, options);
  ...
  scale: function (scale, origin) {
    if (typeof scale === 'number') {
      scale = _leaflet.default.point(scale, scale);
    }
  ...
  _apply: function () {
    ...
    this._matrix = _leaflet.default.matrix(1, 0, 0, 1, 0, 0);
    this._scale = _leaflet.default.point(1, 1);
    this._angle = 0;
  ...
  _getProjectedMatrix: function (angle, scale, rotationOrigin, scaleOrigin) {
    ...
    var matrix = _leaflet.default.matrix(1, 0, 0, 1, 0, 0);

    var origin;
    angle = angle || this._angle || 0;
    scale = scale || this._scale || _leaflet.default.point(1, 1);

    if (!(scale.x === 1 && scale.y === 1)) {
      scaleOrigin = scaleOrigin || this._scaleOrigin;
      origin = map.project(scaleOrigin, zoom);
      matrix = matrix._add(_leaflet.default.matrix(1, 0, 0, 1, origin.x, origin.y))._add(_leaflet.default.matrix(scale.x, 0, 0, scale.y, 0, 0))._add(_leaflet.default.matrix(1, 0, 0, 1, -origin.x, -origin.y));
    }
  ...
  _transformPoints: function (path, angle, scale, rotationOrigin, scaleOrigin) {
    ...
    var projectedMatrix = this._projectedMatrix = this._getProjectedMatrix(angle, scale, rotationOrigin, scaleOrigin); // console.time('transform');
    // all shifts are in-place
    ...
    if (path._point) {
      ...
      var latlngs = path._latlngs;
      path._bounds = new _leaflet.default.LatLngBounds();
  ...
  _createHandlers: function () {
    var map = this._map;
    this._handlersGroup = this._handlersGroup || new _leaflet.default.LayerGroup().addTo(map);
  ...
  _createRotationHandlers: function () {
    ...
    var bottom = new _leaflet.default.LatLng((latlngs[0].lat + latlngs[3].lat) / 2, (latlngs[0].lng + latlngs[3].lng) / 2); // hehe, top is a reserved word

    var topPoint = new _leaflet.default.LatLng((latlngs[1].lat + latlngs[2].lat) / 2, (latlngs[1].lng + latlngs[2].lng) / 2);
    var handlerPosition = map.layerPointToLatLng(_leaflet.default.PathTransform.pointOnLine(map.latLngToLayerPoint(bottom), map.latLngToLayerPoint(topPoint), this.options.handleLength));
    this._handleLine = new _leaflet.default.Polyline([topPoint, handlerPosition], this.options.rotateHandleOptions).addTo(this._handlersGroup);
    ...
    this._rotationOrigin = new _leaflet.default.LatLng((topPoint.lat + bottom.lat) / 2, (topPoint.lng + bottom.lng) / 2);
  ...
  _getRotationOrigin: function () {
    ...
    var lb = latlngs[0];
    var rt = latlngs[2];
    return new _leaflet.default.LatLng((lb.lat + rt.lat) / 2, (lb.lng + rt.lng) / 2);
  },
  ...
  _onScaleStart: function (evt) {
    ...
    this._path.fire('transformstart', {
      ...
      layer: this._path,
      scale: _leaflet.default.point(1, 1)
    });
  ...
  _onScale: function (evt) {
    ...
    this._scale = new _leaflet.default.Point(ratioX, ratioY); // update matrix
  ...
  _getBoundingPolygon: function () {
    if (this._rectShape) {
      return _leaflet.default.GeoJSON.geometryToLayer(this._rectShape, this.options.boundsOptions);
    } else {
      return new _leaflet.default.Rectangle(this._path.getBounds(), this.options.boundsOptions);
    }
  },
  ...
  _createHandler: function (latlng, type, index) {
    ...
    var marker = new HandleClass(latlng, _leaflet.default.Util.extend({}, this.options.handlerOptions, {
  ...
  _onDragEnd: function (evt) {
    ...
    if (!rect.dragging) {
      rect.dragging = new _leaflet.default.Handler.PathDrag(rect);
    }
    ...
    this._path.fire('transformed', {
      scale: _leaflet.default.point(1, 1),
      rotation: 0,
      matrix: _leaflet.default.matrix.apply(undefined, matrix),
      translate: _leaflet.default.point(matrix[4], matrix[5]),
      layer: this._path
    });
  }
});
...
_leaflet.default.Path.addInitHook(function () {
  if (this.options.transform) {
    this.transform = new _leaflet.default.Handler.PathTransform(this, this.options.transform);
  }
});