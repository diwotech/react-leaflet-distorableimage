...
L.DistortableImage.Edit = L.Handler.extend({
  ...
  addHooks: function () {
    ...
    var topLeftScaleHandle = new L.ScaleHandle(overlay, 0); //topLeftScaleHandle.options.icon.style[L.DomUtil.TRANSFORM] += ' rotateZ(90deg)';

    this._scaleHandles.addLayer(topLeftScaleHandle); //this._scaleHandles.addLayer( new L.ScaleHandle(overlay, 0));


    this._scaleHandles.addLayer(new L.ScaleHandle(overlay, 1));

    this._scaleHandles.addLayer(new L.ScaleHandle(overlay, 2));

    this._scaleHandles.addLayer(new L.ScaleHandle(overlay, 3));
  ...
  removeHooks: function () {
    ...
    L.DomEvent.off(overlay._image, 'click', this._showToolbar, this); // First, check if dragging exists;
