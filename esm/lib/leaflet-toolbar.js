...
(function (window, document, undefined) {
  ...
  window.L.Toolbar2 = (L.Layer || L.Class).extend({
    ...
    appendToContainer: function (container) {
      ...
      this._ul = L.DomUtil.create('ul', className, container); // Ensure that clicks, drags, etc. don't bubble up to the map.
  ...
  L.Toolbar2.Popup = L.Toolbar2.extend({
    ...
    _setStyles: function () {
      ...
      this._tipContainer.style.width = toolbarWidth + Math.ceil(parseFloat(L.DomUtil.getStyle(toolbar, 'border-left-width'))) + 'px';
      ...
      tipSize = parseInt(L.DomUtil.getStyle(this._tip, 'width'), 10); // The tip should be anchored exactly where the click event was received.
